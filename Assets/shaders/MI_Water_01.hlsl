...
void InitializeExpressions()
{
	...
	Material.VectorExpressions[1] = float4(0.300000,0.300000,0.300000,0.300000);//(Unknown)
	Material.VectorExpressions[2] = float4(0.300000,0.300000,0.000000,0.000000);//(Unknown)
	Material.VectorExpressions[3] = float4(0.000000,0.000000,0.000000,0.000000);//(Unknown)
	Material.VectorExpressions[4] = float4(0.743763,0.935764,0.739465,0.030000);//Color_1
	Material.VectorExpressions[5] = float4(0.743763,0.935764,0.739465,0.000000);//(Unknown)
	Material.VectorExpressions[6] = float4(0.022313,0.028073,0.022184,0.000000);//(Unknown)
	Material.VectorExpressions[7] = float4(0.050000,0.000000,0.000000,0.000000);//(Unknown)
	Material.ScalarExpressions[0] = float4(0.300000,0.200000,0.000000,25.000000);//MacroNormalIntensity_1 Normal Intensity_1 (Unknown) FadeDistanceColor
	Material.ScalarExpressions[1] = float4(25.000000,0.055000,0.030000,0.250000);//(Unknown) ColorOpacity (Unknown) DepthContribution
	Material.ScalarExpressions[2] = float4(1.000000,0.000000,0.000000,65.000000);//Metallic Specular Roughness FadeDistance
	Material.ScalarExpressions[3] = float4(65.000000,0.750000,0.000000,-0.150000);//(Unknown) Opacity RefractionDepthBias Panning Speed_1
	Material.ScalarExpressions[4] = float4(512.000000,0.100000,1.000000,1.000000);//Wave Size_1 Wave Speed_1 WaveStrength_1 Tiling_1
	Material.ScalarExpressions[5] = float4(1.000000,0.000000,0.000000,0.000000);//Macro Scale_1 (Unknown) (Unknown) (Unknown)
}void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	MaterialFloat Local3 = (1.00000000 + View.MaterialTextureMipBias);
	MaterialFloat Local4 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[1].xy, 0);
	MaterialFloat4 Local5 = UnpackNormalMap(Texture2DSampleBias(Material_Texture2D_0, sampler_Material_Texture2D_0,Parameters.TexCoords[1].xy,MaterialFloat(Local3)));
	MaterialFloat Local6 = MaterialStoreTexSample(Parameters, Local5, 0);
	MaterialFloat Local7 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[2].xy, 0);
	MaterialFloat4 Local8 = UnpackNormalMap(Texture2DSampleBias(Material_Texture2D_0, sampler_Material_Texture2D_0,Parameters.TexCoords[2].xy,MaterialFloat(Local3)));
	MaterialFloat Local9 = MaterialStoreTexSample(Parameters, Local8, 0);
	MaterialFloat3 Local10 = (Local5.rgb + Local8.rgb);
	MaterialFloat3 Local11 = (Local10 * Material.VectorExpressions[2].rgb);
	MaterialFloat3 Local12 = (Local1.rgb + Local11);
	MaterialFloat3 Local13 = lerp(MaterialFloat3(0.00000000,0.00000000,1.00000000),Local12,MaterialFloat(Material.ScalarExpressions[0].y));
	...
	MaterialFloat3 Local14 = lerp(MaterialFloat3(0.00000000,0.00000000,0.00000000),Material.VectorExpressions[3].rgb,MaterialFloat(Material.ScalarExpressions[0].z));
	MaterialFloat Local15 = CalcSceneDepth(ScreenAlignedPosition(GetScreenPosition(Parameters)));
	MaterialFloat Local16 = (Local15 - GetPixelDepth(Parameters));
	MaterialFloat Local17 = (Local16 / Material.ScalarExpressions[1].x);
	MaterialFloat Local18 = min(max(Local17,0.00000000),1.00000000);
	MaterialFloat Local19 = (Material.ScalarExpressions[1].y * Local18);
	MaterialFloat3 Local20 = (Local19 * Material.VectorExpressions[5].rgb);
	MaterialFloat3 Local21 = lerp(Local20,Material.VectorExpressions[6].rgb,MaterialFloat(Material.ScalarExpressions[1].w));
	MaterialFloat Local22 = (Local16 / Material.ScalarExpressions[3].x);
	MaterialFloat Local23 = min(max(Local22,0.00000000),1.00000000);
	MaterialFloat Local24 = (Material.ScalarExpressions[3].y * Local23);

	PixelMaterialInputs.EmissiveColor = Local14;
	PixelMaterialInputs.Opacity = Local24;
	PixelMaterialInputs.OpacityMask = 1.00000000;
	PixelMaterialInputs.BaseColor = Local21;
	PixelMaterialInputs.Metallic = Material.ScalarExpressions[2].x;
	...
	PixelMaterialInputs.Refraction = MaterialFloat2(MaterialFloat2(MaterialFloat2(1.00000000,0.00000000).r,0).r,Material.ScalarExpressions[3].z);
