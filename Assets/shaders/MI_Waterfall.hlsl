...
void InitializeExpressions()
{
	...
	Material.VectorExpressions[2] = float4(0.252897,0.253826,0.300347,1.000000);//Color
	Material.VectorExpressions[3] = float4(0.252897,0.253826,0.300347,0.000000);//(Unknown)
	Material.ScalarExpressions[0] = float4(0.000000,13.694870,13.694870,2.000000);//(Unknown) FadeDistanceColor_1 (Unknown) ColorOpacity_1
	Material.ScalarExpressions[1] = float4(0.110151,500.000000,500.000000,3.000000);//DepthContribution_1 FoamDistance (Unknown) FoamScalar
	Material.ScalarExpressions[2] = float4(40.000000,40.000000,10.000000,0.500000);//FadeDistance_1 (Unknown) Displacement_Power Tesselation_Multiplier
	Material.ScalarExpressions[3] = float4(0.000000,2.000000,8.000000,0.500000);//RefractionDepthBias NormalSize FoamSpeed ScaleUVs
}void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	float3 WorldNormalCopy = Parameters.WorldNormal;

	// Initial calculations (required for Normal)
	MaterialFloat Local0 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[1].xy, 1);
	MaterialFloat4 Local1 = UnpackNormalMap(Texture2DSampleBias(Material_Texture2D_0, sampler_Material_Texture2D_0,Parameters.TexCoords[1].xy,View.MaterialTextureMipBias));
	MaterialFloat Local2 = MaterialStoreTexSample(Parameters, Local1, 1);
	MaterialFloat Local3 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[2].xy, 1);
	MaterialFloat4 Local4 = UnpackNormalMap(Texture2DSampleBias(Material_Texture2D_0, sampler_Material_Texture2D_0,Parameters.TexCoords[2].xy,View.MaterialTextureMipBias));
	MaterialFloat Local5 = MaterialStoreTexSample(Parameters, Local4, 1);
	MaterialFloat3 Local6 = (Local1.rgb + Local4.rgb);
	...
	MaterialFloat3 Local7 = lerp(MaterialFloat3(0.00000000,0.00000000,0.00000000),Material.VectorExpressions[1].rgb,MaterialFloat(Material.ScalarExpressions[0].x));
	MaterialFloat Local8 = CalcSceneDepth(ScreenAlignedPosition(GetScreenPosition(Parameters)));
	MaterialFloat Local9 = (Local8 - GetPixelDepth(Parameters));
	MaterialFloat Local10 = (Local9 / Material.ScalarExpressions[0].z);
	MaterialFloat Local11 = min(max(Local10,0.00000000),1.00000000);
	MaterialFloat Local12 = (Material.ScalarExpressions[0].w * Local11);
	MaterialFloat3 Local13 = (Local12 * Material.VectorExpressions[3].rgb);
	MaterialFloat3 Local14 = lerp(Local13,Material.VectorExpressions[3].rgb,MaterialFloat(Material.ScalarExpressions[1].x));
	MaterialFloat2 Local15 = (Parameters.TexCoords[2].xy * MaterialFloat2(2.00000000,1.00000000));
	MaterialFloat Local16 = MaterialStoreTexCoordScale(Parameters, Local15, 0);
	MaterialFloat4 Local17 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material_Texture2D_1, sampler_Material_Texture2D_1,Local15,View.MaterialTextureMipBias));
	MaterialFloat Local18 = MaterialStoreTexSample(Parameters, Local17, 0);
	MaterialFloat Local19 = (Local9 / Material.ScalarExpressions[1].z);
	MaterialFloat Local20 = min(max(Local19,0.00000000),1.00000000);
	MaterialFloat3 Local21 = (Local17.rgb * Local20);
	MaterialFloat3 Local22 = (Local21 * Material.ScalarExpressions[1].w);
	MaterialFloat3 Local23 = (Local14 * Local22);
	MaterialFloat Local24 = (Local9 / Material.ScalarExpressions[2].y);
	MaterialFloat Local25 = min(max(Local24,0.00000000),1.00000000);
	MaterialFloat Local26 = (0.89999998 * Local25);
	MaterialFloat Local27 = (Local26 * Parameters.VertexColor.a);
	...
	PixelMaterialInputs.Refraction = MaterialFloat2(MaterialFloat2(MaterialFloat2(1.00000000,0.00000000).r,0).r,Material.ScalarExpressions[3].x);
