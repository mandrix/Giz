...
void InitializeExpressions()
{
	...
	Material.VectorExpressions[2] = float4(1.000000,0.923177,0.692708,1.000000);//albedo_color
	Material.VectorExpressions[3] = float4(1.000000,0.923177,0.692708,0.000000);//(Unknown)
	...
	Material.VectorExpressions[8] = float4(27.000000,2.000000,1.000000,0.000000);//(Unknown)
	...
	Material.VectorExpressions[13] = float4(0.100000,0.050000,0.050000,0.050000);//(Unknown)
	Material.VectorExpressions[14] = float4(-1.250000,-0.444000,-0.444000,-0.444000);//(Unknown)
	Material.VectorExpressions[15] = float4(0.670038,0.781250,0.067817,1.000000);//Translucency_Color
	Material.VectorExpressions[16] = float4(0.670038,0.781250,0.067817,0.000000);//(Unknown)
	Material.ScalarExpressions[0] = float4(1.000000,0.000000,-0.100000,1.000000);//Normal_strengh (Unknown) desaturate Brightness
	Material.ScalarExpressions[1] = float4(0.220000,0.850000,0.300000,0.500000);//Specular Roughness Vertex_AO AO_Threshold
	Material.ScalarExpressions[2] = float4(1.000000,0.009174,0.100000,1.000000);//Translucency_Opacity Max Rotation 1 Wind Speed Local Wind Scale 1
	Material.ScalarExpressions[3] = float4(0.050000,27.000000,1.000000,0.000000);//Wind Horizontal Speed  (Unknown) Wind Gust World Scale Wind Angle Offset 1
	Material.ScalarExpressions[4] = float4(-0.444000,-1.250000,-1.250000,-0.444000);//Wind 1 Shelter Dot Product Modulation  Wind 1 Shelter Dot Product Bias  (Unknown) (Unknown)
	Material.ScalarExpressions[5] = float4(2.000000,200.000000,0.250000,0.000000);//Dampening Radius Multiplier 1 Wind 1 Random Rotation Change Rate Wind 1 Random Rotation Influence Wind 1's Angle Contribution to Wind 2
	Material.ScalarExpressions[6] = float4(1.000000,0.100000,0.000000,0.050000);//Local Wind Scale 2 (Unknown) Wind Angle offset 2 Max Rotation 2
	Material.ScalarExpressions[7] = float4(-0.444000,-1.250000,-1.250000,-0.444000);//Wind 2 Shelter Dot Product Modulation  Wind 2 Shelter Dot Product Bias  (Unknown) (Unknown)
	Material.ScalarExpressions[8] = float4(2.000000,200.000000,0.250000,150.000000);//Dampening Radius Multiplier 2 Wind 2 Random Rotation Change Rate Wind 2 Random Rotation Influence Sphere_Mask Radius
	Material.ScalarExpressions[9] = float4(150.000000,0.006667,117.408401,2048.000000);//(Unknown) (Unknown) Object_Scale Wind_Dist_Divide
	Material.ScalarExpressions[10] = float4(2.000000,-1.000000,1.000000,0.000000);//Wind_Distance_Contrast (Unknown) translucency_blend_albedo __SubsurfaceProfile
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	MaterialFloat Local0 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[0].xy, 4);
	...
	MaterialFloat Local2 = MaterialStoreTexSample(Parameters, Local1, 4);
	...
	MaterialFloat3 Local6 = (Local5 * Parameters.TwoSidedSign);
	...
	MaterialFloat3 Local7 = lerp(MaterialFloat3(0.00000000,0.00000000,0.00000000),Material.VectorExpressions[1].rgb,MaterialFloat(Material.ScalarExpressions[0].y));
	MaterialFloat Local8 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[0].xy, 0);
	...
	MaterialFloat3 Local11 = (Material.VectorExpressions[3].rgb * Local9.rgb);
	MaterialFloat Local12 = dot(Local11, MaterialFloat3(0.30000001,0.58999997,0.11000000));
	MaterialFloat3 Local13 = lerp(Local11,MaterialFloat3(Local12,Local12,Local12),MaterialFloat(Material.ScalarExpressions[0].z));
	MaterialFloat3 Local14 = (Local13 * Material.ScalarExpressions[0].w);
	MaterialFloat Local15 = MaterialStoreTexCoordScale(Parameters, Parameters.TexCoords[0].xy, 1);
	MaterialFloat4 Local16 = ProcessMaterialLinearColorTextureLookup(Texture2DSampleBias(Material_Texture2D_2, sampler_Material_Texture2D_2,Parameters.TexCoords[0].xy,View.MaterialTextureMipBias));
	MaterialFloat Local17 = MaterialStoreTexSample(Parameters, Local16, 1);
	MaterialFloat Local18 = (Local16.g * Material.ScalarExpressions[1].y);
	MaterialFloat Local19 = lerp(0.00000000,1.00000000,Parameters.VertexColor.r);
	MaterialFloat Local20 = (Local16.r * Local19);
	MaterialFloat Local21 = (Local20 * Material.ScalarExpressions[1].z);
	MaterialFloat Local22 = min(max(Local21,0.00000000),1.00000000);
	MaterialFloat Local23 = (Local22 + Material.ScalarExpressions[1].w);
	MaterialFloat Local24 = (1.00000000 - Local23);
	MaterialFloat Local25 = min(max(Local24,0.00000000),1.00000000);
	MaterialFloat Local26 = lerp(Local18,1.00000000,Local25);
	MaterialFloat Local27 = (Material.ScalarExpressions[2].x * Local16.b);
	MaterialFloat3 Local298 = (Local9.rgb * Local16.b);
	MaterialFloat3 Local299 = (Local298 * Material.VectorExpressions[16].rgb);
	MaterialFloat3 Local300 = (Local299 * Material.ScalarExpressions[10].z);

	PixelMaterialInputs.EmissiveColor = Local7;
	PixelMaterialInputs.Opacity = Local27;
	...
	PixelMaterialInputs.Subsurface = MaterialFloat4(Local300,Material.ScalarExpressions[10].w);
